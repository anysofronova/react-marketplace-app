const sampleProducts = {
  product1: {
    name: "FURLA Magnifica Eau de Parfum 100ml",
    image: require("../src/img/product1.jpeg"),
    desc: "Magnifica – a joyful shower of mandarin, blue lavender, and juicy peach sensually enlace with a precious rose.",
    price: 76.49,
    status: "availible",
  },
  product2: {
    name: "Jimmy Choo Illicit Flower Eau de Toilette 100ml",
    image: require("../src/img/product2.jpeg"),
    desc: "Illicit Flower is created for a graceful woman, unafraid to toy with the rules.",
    price: 59.99,
    status: "unavailable",
  },
  product3: {
    name: "Montale Soleil De Capri Eau de Parfum 100ml",
    image: require("../src/img/product3.jpeg"),
    desc: "An invigorating harmony of Citrus Fruits from Italy, Grapefruit.",
    price: 94.89,
    status: "availible",
  },
  product4: {
    name: "Narciso Rodriguez Eau de Parfum 90ml",
    image: require("../src/img/product4.jpeg"),
    desc: "For her Eau de Parfum maintains musk as its heart note, just like the other for her perfumes.",
    price: 135.35,
    status: "availible",
  },
  product5: {
    name: "Chloe Signature EDT Eau de Toilette Spray 75ml",
    image: require("../src/img/product5.jpeg"),
    desc: "Chloe Parfum is a gift that she will always remember and one that will cause her to think of the giver fondly for years to come.",
    price: 51.2,
    status: "availible",
  },
  product6: {
    name: "Jean Patou Joy Forever Eau de Toilette Spray 50 ml",
    image: require("../src/img/product6.jpeg"),
    desc: "Sparkling top notes, lush floral mid notes and a luxurious woody base capture the timeless spirit of JOY.",
    price: 110.49,
    status: "availible",
  },
  product7: {
    name: "Versace - Versense Eau De Toilette Spray 100ml",
    image: require("../src/img/product7.jpeg"),
    desc: "The House of Versace is a well known designer brand that always creates only the most exceptional pieces for men and women alike.",
    price: 78.99,
    status: "availible",
  },
  product8: {
    name: "BURBERRY London Eau De Parfum for Women 100ml",
    image: require("../src/img/product8.jpeg"),
    desc: "This fragrance is a floral fresh perfume with gentle top notes of rose and honeysuckle and a deep heart of tiare flower, jasmine, and peony. ",
    price: 102.99,
    status: "availible",
  },
};

export default sampleProducts;
